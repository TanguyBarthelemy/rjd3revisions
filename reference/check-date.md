...
- x:
  ...
  a vector of `Date`, `character`, `integer` or `POSIXt` object
  representing date object

- date_format:

  `character` string (or vector of string) corresponding to the
  format(s) used in `x`
...
If one of the previous formats is recognised, the date will be
assimilated with the first day of the quarter of the year (For example
2000 Q3 is assimilated to 2000-07-01).

The function `check_date_month` checks if the monthly formats. The
...
If one of the previous formats is recognised, the date will be
assimilated with the first day of the month of the year (For example
2000 M3 is assimilated to 2000-03-01).

The function `check_format_date` checks if the object `x` match the
pattern (or one of the patterns) `date_format`.
